    ...
    for i in range(len(boxes)):
        ...
        if i+1 < length:
            ...
            if vertical_dilation:
                ...
                boxes[i][0][1], boxes[i][1][1] = (y_min-top_padding), (y_min-top_padding)
                boxes[i][2][1], boxes[i][3][1] = (y_max+top_padding), (y_max+top_padding)
...
def line_vertical_dilation(line_coordinates, img_height):
...
def top_bottom_padding(cropped_text_region, ratio=1.2):
...
def run_yolo_model(model_weight, image_path, file_name, img_src_save_directory):
    ...
    for info_dict, result_line, cropped_text_region in zip(region_of_interests, result_line_all, cropped_text_region_all_boxes):
        ...
        sorted_line_coordinates = line_vertical_dilation(sorted_line_coordinates, info_dict['img_height'])
    ...
    for idx in range(len(sorted_line_coordinates_allboxes)):
        ...
        for cropped_line_region, result_word in zip(cropped_line_region_allboxes_all_lines[start:end], result_words_allbox_alllines[start:end]):
            ...
            if len(sorted_result_word) != 0:
                ...
                crops = word_horizontal_dilation(boxes, cropped_line_region, vertical_dilation = True, ratio = 1.1)
...
def reconstruct(directory, img_src_save_dir):
    ...
    for file_name in os.listdir(directory):
        ...
        # print(file_name)
        if os.path.isfile(os.path.join(directory, file_name)) \
            and file_name == "dc (1).png": # and file_name == "two_col.png":
